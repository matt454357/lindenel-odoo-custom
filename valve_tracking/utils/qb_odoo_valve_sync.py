...
def qb_sync():
    ...
    write_msg("Getting QB customer references from odoo")
    ...
    write_msg("Getting modified QB customers")
    ...
    sql = """
        ...
        %s
    """ % (cols_string, qb_refs_string)
    write_msg("Getting new QB customers")
    ...
    write_msg("Creating %s new customers" % len(qb_customers))
    ...
    write_msg("Getting QB product references from odoo")
    ...
    sql = """
        ...
        %s
    """ % (cols_string, qb_refs_string)
    write_msg("Getting modified QB products")
    cur.execute(sql, (sync_dt, sync_dt))
    ...
    sql = """
        ...
        %s
    """ % (cols_string, qb_refs_string)
    write_msg("Getting new QB products")
    cur.execute(sql, (sync_dt, ))
    qb_tmpl_rows = cur.fetchall()
    qb_templates = []
    for row in qb_tmpl_rows:
    ...
    write_msg("Creating %s new products" % len(qb_templates))
    ...
    sql = """
        ...
        and TimeCreated<=?
    """ % (cols_string, )
    write_msg("Getting modified QB invoices")
    cur.execute(sql, (sync_dt, sync_dt))
    ...
    write_msg("Getting QB invoice references from odoo")
    ...
    sql = """
        ...
        Where TimeCreated>?
    """ % (cols_string, )
    write_msg("Getting new invoices")
    cur.execute(sql, (sync_dt, ))
    qb_rows = cur.fetchall()
    qb_invoices = []
    qb_customer_ids = []
    ...
    write_msg("Creating %s new invoices" % len(qb_invoices))
    ...
    write_msg("Getting QB invoice line references from odoo")
    ...
    """ % (cols_string, )
    write_msg("Getting new invoice lines")
    cur.execute(sql, (sync_dt, ))
    qb_rows = cur.fetchall()
    qb_inv_lines = []
    qb_inv_ids = []
    ...
    write_msg("Getting QB invoice ref-id map from odoo")
    ...
    write_msg("Creating %s new invoice lines" % len(qb_inv_lines))
...
root.title("QB Odoo Sync")

res = StringVar()

a = Text(root, height=300, width=320, state=DISABLED)
b = Button(root, text="Sync", command=qb_sync)
b.pack()
a.pack()
